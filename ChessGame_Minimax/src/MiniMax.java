...
public class MiniMax {
	...
	public MiniMax(String[] inputData){
		try{
	...
	public Operator MiniMaxDecision(){
		int maxScore = -65535;
		...
		for(int i = 0; i < size; i++)
			for(int j = 0; j < size; j++){
				...
				int score = MinValue(currentBoard, ++height);
				if(score > maxScore){
					maxScore = score;
					char alpha = (char)(65 + j);
					location = Character.toString(alpha) + Integer.toString(i + 1);
	...
	public int MinValue(char[][] state, int height){
		...
		for(int i = 0; i < size; i++)
			for(int j = 0; j < size; j++){
				...
				int score = MaxValue(currentBoard, nextHeight);
				if(score < v){
					//update v to keep it to be the min
					v = score;
				}
			}
	...
	public int MaxValue(char[][] state, int height){
		if(height == depth || isTerminal(state))
			return calculateScore(state);
		int v = -65535;		
		...
		for(int i = 0; i < size; i++)
			for(int j = 0; j < size; j++){
				...
				currentBoard[i][j] = you;
				//max of all the minValues
				int score = MinValue(currentBoard, nextHeight);
				if(score > v){
